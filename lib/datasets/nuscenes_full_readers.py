from lib.utils.nuscenes_utils import generate_dataparser_outputs
...
def readNuScenesFullInfo(path, images='images', split_train=-1, split_test=-1, **kwargs):
    ...
    load_sky_mask = (cfg.mode == 'train') and os.path.exists(sky_mask_dir) and cfg.model.nsg.get('include_sky', False)
    ...
    output = generate_dataparser_outputs(
        ...
        cameras=cfg.data.get('cameras', [0, 1, 2, 3, 4, 5]), # Default 6 cameras
    )
    ...
    for cam in cfg.data.get('cameras', [0, 1, 2, 3, 4, 5]):
    ...
    ########################################################################################################################
    cam_infos = []
    for i in tqdm(range(len(exts)), desc="Loading Cameras"):
        ...
        if load_lidar_depth:
            ...
            if os.path.exists(depth_path):
                ...
                mask = depth['mask']
                value = depth['value']
                depth_map = np.zeros_like(mask).astype(np.float32)
                depth_map[mask] = value
                guidance['lidar_depth'] = depth_map
